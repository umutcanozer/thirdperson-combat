...
public class LockTarget : MonoBehaviour
{
    ...
    {
        if (!_player.HasSword || _player.IsDead) return;
        if (_isLocked)
        {
            UnlockTarget();
            return;
    ...
    {
        if (_targetableObjects.Count <= 1 || !_isLocked || _player.IsDead) return;
    ...
    private void Update()
    {
        ...
        if (_isLocked && !_targetObject.GetComponent<CapsuleCollider>().enabled)
        {
            HandleTargetDeath();
        }
